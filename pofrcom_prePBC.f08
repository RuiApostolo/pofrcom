...
outerloop: do i=IgnoreFirst+1,StepMax
  ...
  innerloop: do j=1,(MolSize-NumFGs*(SizeFG-1))
    ...
    if (abs(array(2) - real(CTFG,dp)) .LE. 0.01_dp ) then
      ...
      if (calc2dpr .eqv. .true.) then
        ! call prcalc2
        results2d = prcalc2d(FGcoms,NumBins,SizeBins)
        ! write 3d results
        ...
        write(81,*) prcalc3d(FGcoms,NumBins,SizeBins)
      end if
end do outerloop
...
!**************************************
! prcalc3d(FGcoms,Numbins,Sizebins)
! FGcom is in the form (1:3,SizeFG), 1:3 being xyz coords
function prcalc3d(innerarray,NB,SB)
implicit none
...
! FGcom is in the form (1:3,SizeFG), 1:3 being xyz coords
function prcalc2d(innerarray,NB,SB)
implicit none
real(dp),intent(in),dimension(:,:) :: innerarray
real(dp),intent(in) :: SB
integer,intent(in) :: NB
